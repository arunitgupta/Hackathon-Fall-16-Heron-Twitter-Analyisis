...
import com.mnp.herontweets.tools.NthLastModifiedTimeTracker;
import com.mnp.herontweets.tools.SlidingWindowCounter;
...
public class RollingCountBolt extends BaseRichBolt {
	...
	public void execute(Tuple tuple) {
		...
		if (TupleHelpers.isTickTuple(tuple)) {
			...
			emitCurrentWindowCounts(source);
		} else {
			countObjAndAck(tuple);
		}
	}

	private void emitCurrentWindowCounts(String s) {
		Map<Object, Long> counts = counter.getCountsThenAdvanceWindow();
		int actualWindowLengthInSeconds = lastModifiedTracker.secondsSinceOldestModification();
		lastModifiedTracker.markAsModified();
		if (actualWindowLengthInSeconds != windowLengthInSeconds) {
			System.out.println("$$$$$$$$$" + String.format(WINDOW_LENGTH_WARNING_TEMPLATE, actualWindowLengthInSeconds,
					windowLengthInSeconds));
		}
		emit(counts,actualWindowLengthInSeconds,s);
	}

	private void emit(Map<Object, Long> counts, int actualWindowLengthInSeconds, String s) {
		for (Entry<Object, Long> entry : counts.entrySet()) {
			Object obj = (Object)entry.getKey();
			Long count = entry.getValue();
			collector.emit(new Values(obj, count, actualWindowLengthInSeconds, s));
		}
	}

	private void countObjAndAck(Tuple tuple) {
		Object obj = tuple.getValue(0);
		source = tuple.getValue(1).toString();
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		// TODO Auto-generated method stub
		declarer.declare(new Fields("obj", "count", "actualWindowLengthInSeconds","src"));
	}
